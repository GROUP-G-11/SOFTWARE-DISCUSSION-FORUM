...
public class AppWindow extends JFrame {
    ...
    public AppWindow(AppContext ctx) {
        ...
        // Apply the app logo at multiple resolutions (crisp in title bar,
        // taskbar, and Alt+Tab). Falls back to the drawn placeholder if the
        // resource is ever missing so the app never fails to launch.
    ...
    /**
     * Loads the app's PNG logo (src/main/resources/icon.png) and scales it
     * down to the standard set of sizes Windows/macOS/Linux expect for
     * title bar, taskbar, and Alt+Tab icons. Returns an empty list if the
     * resource can't be found or read, so the caller can fall back to the
     * drawn placeholder instead of crashing.
     */
    private static List<Image> loadAppIcons() {
        List<Image> icons = new ArrayList<>();
        try (InputStream in = AppWindow.class.getClassLoader().getResourceAsStream("icon.png")) {
            if (in == null) {
                return icons;
            }
            ...
            System.err.println("[AppWindow] Could not load icon.png, falling back to drawn icon: " + e.getMessage());
        }
        return icons;
    }
     ...
     * Kept as a fallback in case icon.png is ever missing from the classpath.
     */
